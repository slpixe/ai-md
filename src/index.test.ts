import { describe, it, expect } from "vitest"; // Explicitly import Vitest functions
...
//
//       // Run CLI from within tempDir so process.cwd() == tempDir
//       // That way relative paths remain short (e.g. 'test1.txt').
//       const { stdout } = await runCLI(`--input . --show-output-files`, { cwd: tempDir });
//
//       // Check stdout for short filenames
...
//
//       // Run the CLI from `tempDir`, using `--input .`
//       const { stdout } = await runCLI(
//           `--input . --ignore-file custom.ignore --show-output-files`,
//           { cwd: tempDir } // ensures aggregator sees tempDir as process.cwd()
//       );
//
//       // Check stdout
...
//
//       // Read codebase.md from inside tempDir
//       const codebasePath = path.join(tempDir, "codebase.md");
//       const content = await fs.readFile(codebasePath, 'utf-8');
//
//       // Verify the content
...
//
//       // Run the CLI from tempDir with `--input .`
//       await runCLI(`--input .`, { cwd: tempDir });
//
//       // Read codebase.md from tempDir
//       const codebasePath = path.join(tempDir, "codebase.md");
//       const content = await fs.readFile(codebasePath, 'utf-8');
//
